...
class MyAnnouncementsPagingSource(
    private val announcementService: AnnouncementService,
    private val token: String
) : PagingSource<Int, Announcement>() {
    val tag = "MyAnnouncementsPagingSource"
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, Announcement> {
        val currentPage = params.key ?: 0
        return try {
            val response = announcementService.getUserAnnouncements(token = token, currentPage = currentPage)
